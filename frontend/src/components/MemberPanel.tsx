...
import { X, Search, Shield, User as UserIcon, Phone, MapPin, Star } from 'lucide-react';
...
export default function MemberPanel({ societyId, onClose }: MemberPanelProps) {
  ...
  // Fetch memberships for this society
  const memberships = MockDb.memberships.filter(m => m.society_id === societyId);
  const users = MockDb.users;
  const currentUser = MockDb.getActiveUser();
  ...
  return (
    <div className="w-68 min-w-[17rem] max-w-[17rem] h-full bg-white flex flex-col border-l border-zinc-200 relative overflow-hidden">
      {/* Header */}
      <div className="px-4 py-4 border-b border-zinc-100 flex justify-between items-center bg-zinc-50/50">
        <h3 className="text-xs font-extrabold text-zinc-500 uppercase tracking-wider">
          Society Directory ({memberships.length})
        </h3>
        <button 
          onClick={onClose}
          className="p-1 text-zinc-400 hover:text-zinc-600 hover:bg-zinc-100 rounded-md btn-transition"
        >
      ...
      <div className="p-3 border-b border-zinc-100 bg-zinc-50/30">
        <div className="relative">
          <input
            ...
            className="w-full pl-8 pr-3 py-1.5 bg-white border border-zinc-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 text-xs font-semibold text-zinc-700"
          />
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1">
        {sortedMembers.map((member) => (
          <div
            ...
            className="flex items-center justify-between p-2 rounded-lg hover:bg-zinc-50 cursor-pointer btn-transition"
          >
      ...
      {selectedMember && selectedMembership && (
        <div className="absolute inset-0 bg-black/40 z-20 flex flex-col justify-end p-4 md:relative md:inset-auto md:bg-transparent md:p-3 md:border-t md:border-zinc-100">
          <div className="bg-white rounded-2xl p-4 shadow-xl border border-zinc-100 md:shadow-none md:border-none relative animate-slide-up md:animate-none">
            <button
      ...
      {showNudgeSheet && selectedMembership && (
        <div className="absolute inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center z-30 p-4">
          <div className="bg-white rounded-3xl w-full p-5 space-y-4 shadow-xl text-center animate-scale-up border border-zinc-100">
            {!nudgeSuccess ? (
              <>
